...
const getStartIndex = (items, years) =>
  (new Date(last(items).start).getFullYear() - years[0]) * 12 + new Date(last(items).start).getMonth() + 1;
const getSpanSize = (items, years) => {
...
const TimelineItem = ({ name, slug, items, years, itemColor }) => {
  const borderColor =
    itemColor === "secondary" ? "border-secondary" : itemColor === "tertiary" ? "border-tertiary" : "border-primary";
  ...
  return (
    <div className={cn("grid")} style={{ gridTemplateColumns: `repeat(${years.length * 12}, minmax(0, 1fr))` }}>
      ...
      {years.map((year, index) => (
        <div
          key={year}
          className={cn("text-sm", "text-center", "pt-2", "border-t", "border-border")}
          style={{
            gridColumn: `span 12 / span 12`,
          }}
        >
          <span className={cn("opacity-75")}>
            {index === years.length - 1 ? "Present" : index % 5 === 0 ? year : ""}
          </span>
        </div>
      ))}
    </div>
